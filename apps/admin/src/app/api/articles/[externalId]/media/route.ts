...
export async function GET(_: NextRequest, { params }: { params: { externalId: string } }) {
  const u = await adminFetch(`/admin/articles/${encodeURIComponent(params.externalId)}/media`, { cache: 'no-store' });
...
export async function POST(req: NextRequest, { params }: { params: { externalId: string } }) {
  ...
  const u = await adminFetch(`/admin/articles/${encodeURIComponent(params.externalId)}/media`, {
